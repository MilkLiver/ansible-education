...
STUDENT_NUMBERS=3
VM_IP=192.168.50.111
EDUCATION_IMAGE_VERSION=2.0.0
...
for ((studentN = 1, portN = 1; studentN <= $STUDENT_NUMBERS; portN += 2, studentN += 1)); do
    ...
    podman run --privileged -e VM_IP=$VM_IP -e ANSIBLE_EDU_ENV=$ANSIBLE_EDU_ENV -e ANSIBLE_EDU_NODE01_PORT=$ANSIBLE_EDU_NODE01_PORT -e ANSIBLE_EDU_NODE02_PORT=$ANSIBLE_EDU_NODE02_PORT -v $WORKSPACE_ROOT_DIR/$ANSIBLE_EDU_ENV:/workspace --network $ANSIBLE_EDU_ENV --network $ANSIBLE_EDU_ENV -idt --name $ANSIBLE_EDU_ENV-controller docker.io/milkliver/ansible-education-controller-container:$EDUCATION_IMAGE_VERSION
    podman run --privileged -e VM_IP=$VM_IP -e ANSIBLE_EDU_ENV=$ANSIBLE_EDU_ENV -e SSH_PORT=$ANSIBLE_EDU_NODE01_PORT --network $ANSIBLE_EDU_ENV -p $ANSIBLE_EDU_NODE01_PORT:22 -idt --name $ANSIBLE_EDU_ENV-node01 docker.io/milkliver/ansible-education-node01-container:$EDUCATION_IMAGE_VERSION
    podman run --privileged -e VM_IP=$VM_IP -e ANSIBLE_EDU_ENV=$ANSIBLE_EDU_ENV -e SSH_PORT=$ANSIBLE_EDU_NODE02_PORT --network $ANSIBLE_EDU_ENV -p $ANSIBLE_EDU_NODE02_PORT:22 -idt --name $ANSIBLE_EDU_ENV-node02 docker.io/milkliver/ansible-education-node02-container:$EDUCATION_IMAGE_VERSION
